        ...
        create: async function () {
          ...
          const createSeamlessGrass = (key, size = 128, seed = 1, density = 180) => {
            ...
            try {
              ...
              try {
                scene.textures.createCanvas(key, size, size).context.drawImage(canvas, 0, 0)
          ...
          for (let i = 0; i < 4; i++) {
            if (scene.textures.exists(`bg${i}`)) availableBgKeys.push(`bg${i}`)
          }
          ...
          scene.setBgForRoom = function (roomInfo) {
            try {
              ...
              if (scene._roomBgCache[roomName]) {
                try {
                  scene.bgFar.setTexture(scene._roomBgCache[roomName])
                } catch (e) {}
                try {
                  scene.bgNear.setAlpha(0)
                  scene.bgFar.setAlpha(1.0)
                } catch (e) {}
                return
              }
              ...
              let chosen = null
              if (roomType && mapping[roomType] && scene.textures.exists(mapping[roomType]))
                chosen = mapping[roomType]
              if (!chosen) {
              ...
              try {
                scene.bgFar.setTexture(chosen)
              } catch (e) {}
              try {
                scene.bgNear.setAlpha(0)
                scene.bgFar.setAlpha(1.0)
                scene.bgNear.setScale(1.02)
              } catch (e) {}
            } catch (e) {}
          }
          ...
          // HP label
          scene.hpLabel = scene.add
            .text(barX - 30, hpY + 1, 'HP', { font: 'bold 14px Arial', fill: '#ff6666' })
            .setDepth(barDepth)
          // HP background
          scene.hpBarBg = scene.add
            .rectangle(barX + barW / 2, hpY + barH / 2, barW, barH, 0x333333)
            .setDepth(barDepth)
            .setStrokeStyle(1, 0x666666)
          scene.hpBarBg.setOrigin(0.5, 0.5)
          // HP fill
          scene.hpBarFill = scene.add
            .rectangle(barX, hpY, barW, barH, 0xcc2222)
            .setDepth(barDepth + 1)
            .setOrigin(0, 0)
          // HP text
          scene.hpText = scene.add
            .text(barX + barW / 2, hpY + barH / 2, '100/100', {
          ...
          // MP label
          scene.mpLabel = scene.add
            .text(barX - 30, mpY + 1, 'MP', { font: 'bold 14px Arial', fill: '#6699ff' })
            .setDepth(barDepth)
          // MP background
          scene.mpBarBg = scene.add
            .rectangle(barX + barW / 2, mpY + barH / 2, barW, barH, 0x333333)
            .setDepth(barDepth)
            .setStrokeStyle(1, 0x666666)
          scene.mpBarBg.setOrigin(0.5, 0.5)
          // MP fill
          scene.mpBarFill = scene.add
            .rectangle(barX, mpY, barW, barH, 0x2244cc)
            .setDepth(barDepth + 1)
            .setOrigin(0, 0)
          // MP text
          scene.mpText = scene.add
            .text(barX + barW / 2, mpY + barH / 2, '100/100', {
              font: 'bold 12px Arial',
              fill: '#ffffff',
            })
            .setDepth(barDepth + 2)
          ...
          // ---------- 货币显示（HP/MP 左侧，金黄色） ----------
          scene.moneyIcon = scene.add
            .text(455, 23, '$', { font: 'bold 18px Arial', fill: '#FFD700' })
            .setDepth(barDepth)
          scene.moneyText = scene.add
            .text(475, 24, '50', { font: 'bold 15px Arial', fill: '#FFD700' })
            .setDepth(barDepth)
          ...
          scene.drawBurnFlame = function (cx, cy, size, layers, timerSec) {
            // 清理旧的火焰
            if (scene._burnFlameGfx) {
              try {
                scene._burnFlameGfx.destroy()
              } catch (e) {}
              scene._burnFlameGfx = null
            }
            if (scene._burnLayersText) {
              try {
                scene._burnLayersText.destroy()
              } catch (e) {}
              scene._burnLayersText = null
            }
            if (scene._burnTimerText) {
              try {
                scene._burnTimerText.destroy()
              } catch (e) {}
              scene._burnTimerText = null
            }

            if (layers <= 0) return // 无烧伤则不绘制

            // 使用 🔥 emoji 作为烧伤图标，固定大小不随层数变化
            const flameText = scene.add
              .text(cx, cy, '🔥', {
                font: '20px Arial',
              })
              .setDepth(120)
            ...
            // 层数标注（火焰右侧）
            const layersText = scene.add
              .text(cx + 10, cy, String(layers), {
            ...
            const timerText = scene.add
              .text(cx, cy + 16, Math.ceil(timerSec) + 's', {
                font: '10px Arial',
                fill: '#FFAA66',
                stroke: '#000000',
                strokeThickness: 2,
          ...
          scene.drawPoisonIcon = function (cx, cy, layers, timerSec) {
            // 清理旧的中毒显示
            if (scene._poisonIconGfx) {
              try {
                scene._poisonIconGfx.destroy()
              } catch (e) {}
              scene._poisonIconGfx = null
            }
            if (scene._poisonLayersText) {
              try {
                scene._poisonLayersText.destroy()
              } catch (e) {}
              scene._poisonLayersText = null
            }
            if (scene._poisonTimerText) {
              try {
                scene._poisonTimerText.destroy()
              } catch (e) {}
              scene._poisonTimerText = null
            }
          ...
          scene.drawBleedIcon = function (cx, cy, layers) {
            // 清理旧的流血显示
            if (scene._bleedIconGfx) {
              try {
                scene._bleedIconGfx.destroy()
              } catch (e) {}
              scene._bleedIconGfx = null
            }
            if (scene._bleedLayersText) {
              try {
                scene._bleedLayersText.destroy()
              } catch (e) {}
              scene._bleedLayersText = null
            }
          ...
          scene.drawSlowIcon = function (cx, cy, timerSec) {
            if (scene._slowIconGfx) {
              try {
                scene._slowIconGfx.destroy()
              } catch (e) {}
              scene._slowIconGfx = null
            }
            if (scene._slowTimerText) {
              try {
                scene._slowTimerText.destroy()
              } catch (e) {}
              scene._slowTimerText = null
            }
          ...
          scene.drawBindIcon = function (cx, cy, timerSec) {
            if (scene._bindIconGfx) {
              try {
                scene._bindIconGfx.destroy()
              } catch (e) {}
              scene._bindIconGfx = null
            }
            if (scene._bindTimerText) {
              try {
                scene._bindTimerText.destroy()
              } catch (e) {}
              scene._bindTimerText = null
            }
          ...
          scene.descText = scene.add.text(20, 50, '', {
            font: '14px Arial',
            fill: '#cccccc',
            wordWrap: { width: 760 },
          })
          ...
          scene.playerLabel = scene.add.text(400 - 20, 320 + 18, 'You', {
            font: '12px Arial',
            fill: '#fff',
          })
          ...
          scene.add.text(
            20,
            560,
            'WASD 移动 | J 攻击/长按蓄力 | Shift+方向+J 突刺 | 空格 互动 | H 月光波 | F 风隐 | G 寒冰风暴',
          ...
          scene.showGameOver = function () {
            ...
            const title = scene.add
              .text(400, 240, '游戏结束', { font: 'bold 48px Arial', fill: '#ff4444' })
              .setOrigin(0.5)
            const hint = scene.add
              .text(400, 310, '你被击败了！点击下方按钮重新开始', {
                font: '20px Arial',
                fill: '#ffffff',
              })
              .setOrigin(0.5)
          ...
          scene.renderRoom = function (roomInfo) {
            ...
            const maxHp =
              roomInfo.playerMaxHp !== undefined ? roomInfo.playerMaxHp : scene.playerStats.maxHp
            const mp = roomInfo.playerMp !== undefined ? roomInfo.playerMp : scene.playerStats.mp
            const maxMp =
              roomInfo.playerMaxMp !== undefined ? roomInfo.playerMaxMp : scene.playerStats.maxMp
            const money =
              roomInfo.playerMoney !== undefined ? roomInfo.playerMoney : scene.playerStats.money
            try {
            ...
            scene.altarsData = []
            scene.altarIndicators.forEach((ind) => {
              try {
                ind.destroy()
              } catch (e) {}
            })
            scene.altarIndicators = []
            if (scene.altarGlowGraphics) {
              scene.altarGlowGraphics.clear()
            }
            if (scene.wisdomOverlay) {
              try {
                scene.wisdomOverlay.destroy()
              } catch (e) {}
              scene.wisdomOverlay = null
            }
            // 清理商店覆盖层和NPC状态
            if (scene.shopMenuOverlay) {
              try {
                scene.shopMenuOverlay.destroy()
              } catch (e) {}
              scene.shopMenuOverlay = null
            }
            if (scene.shopBuyOverlay) {
              try {
                scene.shopBuyOverlay.destroy()
              } catch (e) {}
              scene.shopBuyOverlay = null
            }
            if (scene.shopSellOverlay) {
              try {
                scene.shopSellOverlay.destroy()
              } catch (e) {}
              scene.shopSellOverlay = null
            }
            if (scene.shopNpcCircle) {
              try {
                scene.shopNpcCircle.destroy()
              } catch (e) {}
            ...
            scene.shopIndicators.forEach((ind) => {
              try {
                ind.destroy()
              } catch (e) {}
            })
            ...
            if (scene.encounterGroup) {
              scene.encounterGroup.clear(true, true)
            }
            scene.encounterData = null
            scene.encounterIndicators.forEach((ind) => {
              try {
                ind.destroy()
              } catch (e) {}
            })
            ...
            if (roomChanged) {
              try {
                scene.setBgForRoom && scene.setBgForRoom(roomInfo)
              } catch (e) {}
            ...
            // ---------- 将背景缩放到房间矩形内并裁剪 ----------
            try {
              scene.fitBgToRoom && scene.fitBgToRoom(rectLeft, rectTop, roomW, roomH)
            } catch (e) {}
            ...
            // 房间矩形外填充深色背景
            if (!scene._darkBg) {
              scene._darkBg = scene.add
                .rectangle(400, 300, 800, 600, 0x111111)
                .setDepth(-1)
                .setOrigin(0.5)
            }

            if (!scene.doorRects) scene.doorRects = []
            scene.doorRects.forEach((d) => {
              try {
                d.rect.destroy()
                d.label.destroy()
              } catch (e) {}
            ...
            exits.forEach((dir) => {
              ...
              if (dir === 'north') {
                posx = rectCenterX
                posy = rectTop - h / 2 - outsideOffset
              }
              if (dir === 'south') {
                posx = rectCenterX
                posy = rectTop + roomH + h / 2 + outsideOffset
              }
              if (dir === 'west') {
                posx = rectLeft - w / 2 - outsideOffset
                posy = rectCenterY
              }
              if (dir === 'east') {
                posx = rectLeft + roomW + w / 2 + outsideOffset
                posy = rectCenterY
              }

              const rect = scene.add
                .rectangle(posx, posy, w, h, 0x664422)
                .setStrokeStyle(2, 0x222222)
              const label = scene.add.text(posx - 20, posy - 10, dir.toUpperCase(), {
                font: '14px Arial',
                fill: '#ffffff',
              })
            ...
            scene._roomBounds = {
              left: rectLeft,
              top: rectTop,
              right: rectLeft + roomW,
              bottom: rectTop + roomH,
            }

            // 更新 Buff/Debuff 显示（基于后端 activeEffects）
            try {
              scene.updateBuffDisplay(roomInfo.activeEffects)
            } catch (e) {}
            ...
            if (scene._lastMoveDir && scene._lastRoomChanged) {
              ...
              if (to && exits.includes(to)) {
                ...
                if (to === 'north') {
                  targetX = rectCenterX
                  targetY = rectTop + th / 2 + 8
                }
                if (to === 'south') {
                  targetX = rectCenterX
                  targetY = rectTop + roomH - th / 2 - 8
                }
                if (to === 'west') {
                  targetX = rectLeft + tw / 2 + 8
                  targetY = rectCenterY
                }
                if (to === 'east') {
                  targetX = rectLeft + roomW - tw / 2 - 8
                  targetY = rectCenterY
                }
            ...
            items.forEach((item) => {
              ...
              const label = scene.add.text(x - 24, y + 24, item.name, {
                font: '14px Arial',
                fill: '#fff',
              })
              scene.itemsGroup.add(circle)
              scene.itemsGroup.add(label)
              scene.itemsData.push({
                name: item.name,
                x,
                y,
                circle,
            ...
            sortedMonsters.forEach((mon) => {
              ...
              const label = scene.add.text(x - 32, y + 34, labelText, {
                font: '14px Arial',
                fill: '#fff',
              })
              ...
              const hpBg = scene.add
                .rectangle(hpBarX + hpBarW / 2, hpBarY + hpBarH / 2, hpBarW, hpBarH, 0x333333)
                .setStrokeStyle(1, 0x666666)
              // 血条填充
              ...
              const hpFill = scene.add
                .rectangle(hpBarX, hpBarY, hpBarW * hpRatio, hpBarH, hpColor)
                .setOrigin(0, 0)
              // HP 数字文字（血条内部居中，加粗白字带描边）
              const hpNumText = scene.add
                .text(hpBarX + hpBarW / 2, hpBarY + hpBarH / 2, mon.hp + '/' + monMaxHp, {
              ...
              scene.monstersData.push({
                ...
                hpBarW: hpBarW,
                hpBarH: hpBarH,
                defense: mon.defense || 0,
                magicResist: mon.magicResist || 0,
                speed: mon.speed || 100,
                effectiveSpeed:
                  mon.effectiveSpeed !== undefined ? mon.effectiveSpeed : mon.speed || 100,
            ...
            if (roomType === 'CAMPFIRE' && altarsFromBackend.length > 0) {
              ...
              for (let ai = 0; ai < altarOrder.length; ai++) {
                ...
                const rect = scene.add
                  .rectangle(ax, ay, altarSize, altarSize, color)
                  .setStrokeStyle(2, isActivated ? 0xffffff : 0x444444)
                const typeLabel = scene.add
                  .text(ax, ay + altarSize / 2 + 8, altarInfo.displayName || typeName, {
                    font: '11px Arial',
                    fill: '#ffffff',
                  })
                  .setOrigin(0.5, 0)
            ...
            const showEncounter =
              isEncounter && randomEvent && (!randomEvent.used || randomEvent.type === 'FOUNTAIN')
            if (showEncounter) {
              ...
              let eventSprite
              if (
                eventType === 'WOODEN_CHEST' ||
                eventType === 'GOLDEN_CHEST' ||
                eventType === 'CHEST'
              ) {
                // 使用实体绘制替换基础方块
                ...
                // 喷泉：蓝色圆形+内部波浪
                eventSprite = scene.add
                  .circle(ex, ey, eSize / 2, eventColor)
                ...
                // 通用事件：彩色方块
                eventSprite = scene.add
                  .rectangle(ex, ey, eSize, eSize, eventColor)
                  .setStrokeStyle(3, 0xffffff)
              }
              ...
              if (eventType !== 'ELITE_ENEMY') {
                eventLabel = scene.add
                  .text(ex - 30, ey + eSize / 2 + 8, eventDisplayName, {
                    font: '12px Arial',
                    fill: '#ffffff',
                    stroke: '#000000',
                    strokeThickness: 2,
            ...
            droppedItems.forEach((drop) => {
              ...
              if (scene._dropPosCache[cacheKey]) {
                ...
                // 基于房间名+物品名生成确定性偏移
                const hash =
                  cacheKey.split('').reduce((acc, c) => acc * 31 + c.charCodeAt(0), 0) & 0x7fffffff
              ...
              const label = scene.add
                .text(dx - 20, dy + 16, drop.itemName, {
                  font: '11px Arial',
                  fill: '#ffffff',
                  stroke: '#000000',
                  strokeThickness: 2,
            ...
            try {
              window.dispatchEvent(
                new CustomEvent('minimap:update', { detail: { roomName: roomInfo.name || '' } }),
              )
            } catch (e) {}
          }
          ...
          scene.showWisdomOverlay = function () {
            ...
            if (boons.length === 0) {
              ...
              const title = scene.add
                .text(400, 280, '博学祭坛 — 无可用增益', {
                  font: 'bold 22px Arial',
                  fill: '#4488ff',
                })
                .setOrigin(0.5)
              const hint = scene.add
                .text(400, 340, '（增益数据未加载，请重试）', {
                  font: '14px Arial',
                  fill: '#888888',
                })
                .setOrigin(0.5)
            ...
            boons.forEach((boon, i) => {
              ...
              const txt = scene.add
                ...
                .setInteractive({ useHandCursor: true })
              txt.on('pointerover', () =>
                txt.setStyle({ fill: '#ffffff', backgroundColor: '#334488' }),
              )
              txt.on('pointerout', () =>
                txt.setStyle({ fill: '#dddddd', backgroundColor: '#222244' }),
              )
              txt.on('pointerdown', () => {
                // 发送选择命令到后端
          ...
          scene.showShopMenu = function () {
            ...
            buyBtn.on('pointerover', () =>
              buyBtn.setStyle({ fill: '#ffffff', backgroundColor: '#445544' }),
            )
            buyBtn.on('pointerout', () =>
              buyBtn.setStyle({ fill: '#44cc44', backgroundColor: '#222222' }),
            )
            ...
            sellBtn.on('pointerover', () =>
              sellBtn.setStyle({ fill: '#ffffff', backgroundColor: '#445544' }),
            )
            sellBtn.on('pointerout', () =>
              sellBtn.setStyle({ fill: '#FFD700', backgroundColor: '#222222' }),
            )
            ...
            closeBtn.on('pointerover', () =>
              closeBtn.setStyle({ fill: '#ffffff', backgroundColor: '#664444' }),
            )
            closeBtn.on('pointerout', () =>
              closeBtn.setStyle({ fill: '#ff6666', backgroundColor: '#222222' }),
            )
          ...
          scene.showShopBuy = function () {
            ...
            for (let i = 0; i < shopItems.length; i++) {
              ...
              const icon = scene.add
                .rectangle(cx, cy, iconSize, iconSize, 0x888888)
                .setStrokeStyle(2, 0xaaaaaa)
              // 商品名称
              const nameText = scene.add
                .text(cx, cy + iconSize / 2 + 8, item.name, {
                  font: '13px Arial',
                  fill: '#ffffff',
                })
                .setOrigin(0.5, 0)
              ...
              const priceText = scene.add
                .text(cx, cy + iconSize / 2 + 28, '$' + item.price, {
                  font: 'bold 14px Arial',
                  fill: '#FFD700',
                })
                .setOrigin(0.5, 0)
              ...
              const hitArea = scene.add
                .rectangle(cx, cy, itemW - 10, itemH - 10, 0x000000, 0)
                .setInteractive({ useHandCursor: true })
              ...
              hitArea.on('pointerdown', async () => {
                ...
                try {
                  ...
                  if (j && j.status === 'success') {
                    ...
                    if (j.data) {
                      const hp =
                        j.data.playerHp !== undefined ? j.data.playerHp : scene.playerStats.hp
                      const maxHp =
                        j.data.playerMaxHp !== undefined
                          ? j.data.playerMaxHp
                          : scene.playerStats.maxHp
                      const mp =
                        j.data.playerMp !== undefined ? j.data.playerMp : scene.playerStats.mp
                      const maxMp =
                        j.data.playerMaxMp !== undefined
                          ? j.data.playerMaxMp
                          : scene.playerStats.maxMp
                      const money =
                        j.data.playerMoney !== undefined
                          ? j.data.playerMoney
                          : scene.playerStats.money
                      try {
                        scene.updatePlayerBars(hp, maxHp, mp, maxMp, money)
                      } catch (e) {}
                    }
            ...
            endBtn.on('pointerover', () =>
              endBtn.setStyle({ fill: '#ffffff', backgroundColor: '#664444' }),
            )
            endBtn.on('pointerout', () =>
              endBtn.setStyle({ fill: '#ff6666', backgroundColor: '#222222' }),
            )
            ...
            overlay.add([bg, title, ...itemElements, endBtn])
          ...
          scene.showShopSell = function () {
            ...
            const moneyText = scene.add
              .text(400, 110, '当前金币：$' + currentMoney, {
                font: '16px Arial',
                fill: '#FFD700',
              })
              .setOrigin(0.5)
            ...
            async function loadAndShowSellItems() {
              try {
                ...
                for (let i = 0; i < items.length; i++) {
                  ...
                  sellBtn.on('pointerout', () =>
                    sellBtn.setStyle({ fill: '#44cc44', backgroundColor: '#222222' }),
                  )

                  // 已装备标记（不可出售）
                  if (invItem.equipped) {
                    const equippedTag = scene.add
                      .text(580, y, '已装备', {
                        font: '13px Arial',
                        fill: '#ff8800',
                      })
          ...
          scene.keys = scene.input.keyboard.addKeys(
            'W,A,S,D,SHIFT,J,SPACE,H,F,G,ONE,TWO,THREE,FOUR,FIVE',
          )
          ...
          scene.drawArcSlash = (gfx, progress, alpha = 1) => {
            ...
            for (let i = 0; i <= segs; i++) {
              ...
              glowOuter.push({
                x: scene.player.x + Math.cos(ang) * outerR * 1.1,
                y: scene.player.y + Math.sin(ang) * outerR * 1.1,
              })
            }
            ...
            for (let i = 0; i <= segs; i++) {
              ...
              const ang = startAngle + (endAngle - startAngle) * t
              outer.push({
                x: scene.player.x + Math.cos(ang) * outerR,
                y: scene.player.y + Math.sin(ang) * outerR,
              })
            }
            for (let i = segs; i >= 0; i--) {
              ...
              const ang = startAngle + (endAngle - startAngle) * t
              inner.push({
                x: scene.player.x + Math.cos(ang) * innerR,
                y: scene.player.y + Math.sin(ang) * innerR,
              })
            }
            ...
            for (let i = segs; i >= 0; i--) {
              ...
              const ang = startAngle + (endAngle - startAngle) * t
              inner2.push({
                x: scene.player.x + Math.cos(ang) * innerR * 0.6,
                y: scene.player.y + Math.sin(ang) * innerR * 0.6,
              })
            }
          ...
          scene.drawFireDistortion = (gfx, progress) => {
            const startAngle =
              scene.facingAngle - Phaser.Math.DegToRad(scene.attackConfig.angleDeg / 2)
            const endAngle =
              startAngle + Phaser.Math.DegToRad(scene.attackConfig.angleDeg) * progress
            const baseR = scene.attackConfig.radius
            gfx.clear()
            for (let layer = 0; layer < 4; layer++) {
              const offset =
                Math.sin(progress * Math.PI * 3 + layer * 2) * 0.12 + (Math.random() - 0.5) * 0.06
              ...
              for (let i = 0; i <= 24; i++) {
                ...
                pts.push({
                  x: scene.player.x + Math.cos(ang) * r,
                  y: scene.player.y + Math.sin(ang) * r,
                })
              }
          ...
          scene.spawnAttackParticles = (progress, count) => {
            ...
            for (let i = 0; i < count; i++) {
              const ang =
                endAngle +
                (Math.random() - 0.5) * Phaser.Math.DegToRad(scene.attackConfig.angleDeg) * 0.8
          ...
          window.__zuul_game_update_handler = function (e) {
            ...
            if (j && j.data) {
              ...
              const maxHp =
                j.data.playerMaxHp !== undefined ? j.data.playerMaxHp : scene.playerStats.maxHp
              const mp = j.data.playerMp !== undefined ? j.data.playerMp : scene.playerStats.mp
              const maxMp =
                j.data.playerMaxMp !== undefined ? j.data.playerMaxMp : scene.playerStats.maxMp
              const money =
                j.data.playerMoney !== undefined ? j.data.playerMoney : scene.playerStats.money
              try {
                scene.updatePlayerBars(hp, maxHp, mp, maxMp, money)
              } catch (e) {}
              // 更新 Buff/Debuff 显示
            ...
            if (
              !scene._backpackPaused &&
              !scene._controlPanelPaused &&
              j &&
              j.data &&
              j.data.name &&
              !scene.shopMenuOverlay &&
          ...
          this.sys.events.on('update', function (time, delta) {
              ...
              ;(async () => {
                try {
                  ...
                  if (j && j.data) {
                    ...
                    // 同步风隐状态
                    if (
                      j.data.windCloakActive !== undefined &&
            ...
            try {
              if (
                ...
                scene.keys.H.isDown &&
                hasEnoughMp &&
                !scene._waveCharging.active &&
                !scene._wavePendingSend &&
                !scene.shopMenuOverlay &&
                !scene.shopBuyOverlay &&
                !scene.shopSellOverlay &&
                !scene.wisdomOverlay
            ...
            const fireWave = () => {
              ...
              try {
                if (scene._waveCharging.chargeBarGfx) {
                  scene._waveCharging.chargeBarGfx.destroy()
                  scene._waveCharging.chargeBarGfx = null
                }
                if (scene._waveCharging.directionGfx) {
                  scene._waveCharging.directionGfx.destroy()
                  scene._waveCharging.directionGfx = null
                }
              } catch (e) {}
              ...
              const proj = scene.spawnWaveProjectile(
                scene.player.x,
                scene.player.y,
                scene.facingAngle,
                rbWave,
            ...
            if (scene._waveCharging.active) {
              try {
                ...
                if (!hDown) {
                  ...
                  const baseLX =
                    scene.player.x +
                    Math.cos(scene.facingAngle) * triBaseDist +
                    Math.cos(perpLeft) * triHalfWidth
                  const baseLY =
                    scene.player.y +
                    Math.sin(scene.facingAngle) * triBaseDist +
                    Math.sin(perpLeft) * triHalfWidth
                  const baseRX =
                    scene.player.x +
                    Math.cos(scene.facingAngle) * triBaseDist +
                    Math.cos(perpRight) * triHalfWidth
                  const baseRY =
                    scene.player.y +
                    Math.sin(scene.facingAngle) * triBaseDist +
                    Math.sin(perpRight) * triHalfWidth
            ...
            // ---- 风隐已激活时，再按F键立即解除 ----
            if (
              scene._windCloakActive &&
              scene.keys.F &&
              Phaser.Input.Keyboard.JustDown(scene.keys.F)
            ) {
            ...
            if (!scene._windCloakActive) {
              try {
                if (
                  scene.keys.F &&
                  scene.keys.F.isDown &&
                  !scene._windCloakCharging.active &&
                  !scene._windCloakPending &&
                  !scene._waveCharging.active &&
                  !scene._chargeAttack.active &&
                  !scene.shopMenuOverlay &&
                  !scene.shopBuyOverlay &&
                  !scene.shopSellOverlay &&
                  !scene.wisdomOverlay
                  ...
                  if (scene._windCloakCharging.charged && !scene._windCloakPending) {
                    ...
                    try {
                      if (scene._windCloakCharging.chargeBarGfx) {
                        scene._windCloakCharging.chargeBarGfx.destroy()
                        scene._windCloakCharging.chargeBarGfx = null
                      }
                    } catch (e) {}
                  } else {
                    // 未满蓄力松开 → 取消
            ...
            try {
              if (
                ...
                scene.keys.G.isDown &&
                hasIceStormMp &&
                !scene._iceStormCharging.active &&
                !scene._iceStormPending &&
                !scene._waveCharging.active &&
                !scene._windCloakCharging.active &&
                !scene._chargeAttack.active &&
                !scene.shopMenuOverlay &&
            ...
            const fireIceStorm = () => {
              ...
              try {
                if (scene._iceStormCharging.chargeBarGfx) {
                  scene._iceStormCharging.chargeBarGfx.destroy()
              ...
              const frostRing = scene.add
                .circle(scene.player.x, scene.player.y, 20, 0x88ccff, 0)
                .setDepth(200)
              frostRing.setStrokeStyle(4, 0x88ccff)
              scene.tweens.add({
                targets: frostRing,
                radius: ICE_VFX_RADIUS,
                alpha: 0,
                duration: 600,
                ease: 'Cubic.easeOut',
                onUpdate: () => {
                  frostRing.setStrokeStyle(3, 0x88ccff, frostRing.alpha)
                },
                onComplete: () => {
                  try {
                    frostRing.destroy()
              ...
              scene.time.delayedCall(150, () => {
                const frostRing2 = scene.add
                  .circle(scene.player.x, scene.player.y, 20, 0xaaddff, 0)
                  .setDepth(200)
                frostRing2.setStrokeStyle(3, 0xaaddff)
                scene.tweens.add({
                  targets: frostRing2,
                  radius: ICE_VFX_RADIUS * 1.1,
                  alpha: 0,
                  duration: 500,
                  ease: 'Cubic.easeOut',
                  onUpdate: () => {
                    frostRing2.setStrokeStyle(2, 0xaaddff, frostRing2.alpha)
                  },
                  onComplete: () => {
                    try {
                      frostRing2.destroy()
              ...
              for (let i = 0; i < 30; i++) {
                ...
                const px = scene.player.x + Math.cos(ang) * dist
                const py = scene.player.y + Math.sin(ang) * dist
                const iceSpark = scene.add
                  .circle(px, py, 1 + Math.random() * 3, Math.random() < 0.3 ? 0xffffff : 0x88ccff)
                  .setDepth(201)
                scene.tweens.add({
                  targets: iceSpark,
              ...
              const frostFlash = scene.add
                .rectangle(400, 300, 800, 600, 0x88ccff, 0.15)
                .setDepth(500)
              scene.tweens.add({
                targets: frostFlash,
                alpha: 0,
              ...
              const frostOverlay = scene.add
                .circle(scene.player.x, scene.player.y, ICE_VFX_RADIUS, 0x6688bb, 0.12)
                .setDepth(0)
              scene.tweens.add({
                targets: frostOverlay,
                alpha: 0,
                duration: 2000,
                delay: 300,
                onComplete: () => {
            ...
            const isIceStormCharging =
              scene._iceStormCharging.active && !scene._iceStormCharging.charged // 寒冰风暴蓄力中禁止移动
            ...
            if (!isWaveCharging && !isIceStormCharging && !scene._piercing) {
              ...
              if (vx !== 0 || vy !== 0) {
                ...
                if (!isWaveAiming) {
                  ...
                  let hasBind = false
                  try {
                    const bindEff = (scene._activeEffects || []).find(
                      (e) => e && e.type === 'BIND' && e.layers > 0,
                    )
                  ...
                  if (!hasBind) {
                    ...
                    let hasSlow = false
                    try {
                      const slowEff = (scene._activeEffects || []).find(
                        (e) => e && e.type === 'SLOW' && e.layers > 0,
                      )
            ...
            for (const mon of scene.monstersData) {
              ...
              if (dist <= attackRange && !isSlime) {
                ...
                // 每个怪物独立冷却，无全局锁
                if (
                  !scene.monsterAttackCooldowns[mon.name] ||
                  now - scene.monsterAttackCooldowns[mon.name] >= attackCooldown
                ) {
                ...
                dist <= (isElite ? ELITE_DETECT_RANGE : MONSTER_DETECT_RANGE) ||
                mon.type === 2
              ) {
                ...
                // 更新怪物精灵位置
                try {
                  mon.circ.setPosition(mon.x, mon.y)
                } catch (e) {}
                try {
                  mon.label.setPosition(mon.x - 32, mon.y + 24)
                } catch (e) {}
              }
              ...
              if (isSlime && dist <= (scene.playerRadius || 10) + 20) {
                ...
                if (
                  !scene.monsterAttackCooldowns[mon.name] ||
                  now - scene.monsterAttackCooldowns[mon.name] >= slimeCooldown
                ) {
                  scene.monsterAttackCooldowns[mon.name] = now
                  ;(async () => {
                    try {
            ...
            if (scene._jHeldSince && !(scene.keys.J && scene.keys.J.isDown)) {
              ...
              if (
                heldDuration < CHARGE_ENTRY_THRESHOLD &&
                !scene._attackOnCooldown &&
                !scene._chargeAttack.active
              ) {
                // 短按（<200ms）→ 触发横扫或突刺
                ...
                let pierceStartX = scene.player.x,
                  pierceStartY = scene.player.y
                let pierceEndX = scene.player.x,
                  pierceEndY = scene.player.y

                if (isShiftMove) {
                  // 突刺特效
                  const startX = scene.player.x,
                    startY = scene.player.y
                  const dx_ = Math.cos(scene.facingAngle),
                    dy_ = Math.sin(scene.facingAngle)
                  ...
                  const targetX = Phaser.Math.Clamp(
                    startX + dx_ * actualDist,
                    rb.left + pr_,
                    rb.right - pr_,
                  )
                  const targetY = Phaser.Math.Clamp(
                    startY + dy_ * actualDist,
                    rb.top + pr_,
                    rb.bottom - pr_,
                  )
                  ...
                  scene.tweens.add({
                    ...
                    x: targetX,
                    y: targetY,
                    duration: cfg.pierceDuration || 100,
                    ease: 'Cubic.easeOut',
                  })
                  const g2 = scene.add.graphics()
                  const extra = cfg.pierceDistanceExpand || 1.0
                  const effTX = Phaser.Math.Clamp(
                    startX + dx_ * actualDist * extra,
                    rb.left + pr_,
                    rb.right - pr_,
                  )
                  const effTY = Phaser.Math.Clamp(
                    startY + dy_ * actualDist * extra,
                    rb.top + pr_,
                    rb.bottom - pr_,
                  )
                  const mx_ = (startX + effTX) / 2,
                    my_ = (startY + effTY) / 2
                  ...
                  scene.tweens.add({
                    targets: progress_,
                    t: 1,
                    duration: scene.attackConfig.sweepDuration || 160,
                    ease: 'Cubic.easeOut',
                    onUpdate: () => {
                      scene.drawArcSlash(mainGfx, progress_.t, 0.95)
                      scene.drawFireDistortion(fireGfx, progress_.t)
                    },
                    onComplete: () => {
                ...
                ;(async () => {
                  try {
                    ...
                    if (isShiftMove) {
                      ...
                      requestBody = {
                        attackType: 'pierce',
                        playerX: pierceStartX,
                        playerY: pierceStartY,
            ...
            try {
              if (
                ...
                !scene._attackOnCooldown &&
                !scene._waveCharging.active &&
                !scene._chargeAttackPendingSend &&
                !scene.shopMenuOverlay &&
                !scene.shopBuyOverlay &&
                !scene.shopSellOverlay &&
                !scene.wisdomOverlay &&
                scene._jHeldSince &&
                chargeNowMs - scene._jHeldSince >= CHARGE_ENTRY_THRESHOLD
              ) {
                ...
                if (!jDown) {
                  ...
                  if (scene._chargeAttack.charged) {
                    ...
                    if (!scene._chargeAttackPendingSend) {
                      ...
                      // 清理蓄力UI
                      try {
                        if (scene._chargeAttack.chargeBarGfx) {
                          scene._chargeAttack.chargeBarGfx.destroy()
                          scene._chargeAttack.chargeBarGfx = null
                        }
                        if (scene._chargeAttack.circleGfx) {
                          scene._chargeAttack.circleGfx.destroy()
                      ...
                      const drawChargeCircle = (gfx, alpha) => {
                        ...
                        for (let s = 0; s <= segs; s++) {
                          ...
                          outer.push({
                            x: scene.player.x + Math.cos(a) * CHARGE_VFX_RADIUS,
                            y: scene.player.y + Math.sin(a) * CHARGE_VFX_RADIUS,
                          })
                        }
                        for (let s = segs; s >= 0; s--) {
                          const a = (s / segs) * Math.PI * 2
                          inner.push({
                            x: scene.player.x + Math.cos(a) * CHARGE_VFX_RADIUS * 0.35,
                            y: scene.player.y + Math.sin(a) * CHARGE_VFX_RADIUS * 0.35,
                          })
                        }
                      ...
                      const shockwave1 = scene.add
                        .circle(scene.player.x, scene.player.y, 20, 0xffffff, 0)
                        .setDepth(16)
                      shockwave1.setStrokeStyle(3, 0xff8800)
                      scene.tweens.add({
                        targets: shockwave1,
                        radius: CHARGE_VFX_RADIUS * 1.15,
                        alpha: 0,
                        duration: 250,
                        ease: 'Cubic.easeOut',
                        onUpdate: () => {
                          shockwave1.setStrokeStyle(3, 0xff8800, shockwave1.alpha)
                        },
                        onComplete: () => {
                          shockwave1.destroy()
                        },
                      ...
                      scene.time.delayedCall(60, () => {
                        ...
                        scene.tweens.add({
                          ...
                          ease: 'Cubic.easeOut',
                          onUpdate: () => {
                            shockwave2.setStrokeStyle(3, 0xff6600, shockwave2.alpha)
                          },
                          onComplete: () => {
                            shockwave2.destroy()
                          },
                      ...
                      for (let i = 0; i < 25; i++) {
                        ...
                        const spark = scene.add
                          .circle(
                            ...
                            py,
                            1.5 + Math.random() * 3,
                            Math.random() < 0.4 ? 0xffee88 : 0xff4400,
                          )
                          .setDepth(17)
                        scene.tweens.add({
                          targets: spark,
                          x: px + Math.cos(ang) * (60 + Math.random() * 60),
                  ...
                  // 蓄力中（未满）→ 绘制进度条和蓄力圈
                  if (!scene._chargeAttack.chargeBarGfx) {
                    scene._chargeAttack.chargeBarGfx = scene.add.graphics().setDepth(150)
                  }
                  if (!scene._chargeAttack.circleGfx) {
                    scene._chargeAttack.circleGfx = scene.add.graphics().setDepth(14)
                  }
                  ...
                  const fR = Math.floor(180 + progress * 75),
                    fG = Math.floor(80 + progress * 100),
                    fB = 20
                  ...
                  if (progress >= 1) {
                    scene._chargeAttack.charged = true
                  }
                } else {
                  // 已满蓄力 → 保持满蓄力UI脉冲
                  if (!scene._chargeAttack.chargeBarGfx) {
                  ...
                  for (let d = 0; d < 24; d++) {
                    ...
                    circleGfx.arc(
                      ...
                      ((d + 0.4) / 24 + chargeNowMs * 0.0005) * Math.PI * 2,
                      false,
                    )
                    circleGfx.strokePath()
                  }
            ...
            for (const proj of scene._waveProjectiles) {
              ...
              if (bounced) {
                ...
                proj.hitMonsters = new Set()
                // bounce flash
                const bounceFlash = scene.add
                  .circle(proj.x, proj.y, proj.radius * 2, 0xffd700, 0.5)
                  .setDepth(201)
                scene.tweens.add({
                  targets: bounceFlash,
                  alpha: 0,
              ...
              if (proj.bounces > proj.maxBounces) {
                proj.alive = false
                // death flash
                const deathFlash = scene.add
                  .circle(proj.x, proj.y, proj.radius * 3, 0xffffaa, 0.6)
                  .setDepth(202)
                scene.tweens.add({
                  targets: deathFlash,
                  alpha: 0,
            ...
            const hasAliveMonsters =
              scene.monstersData && scene.monstersData.some((m) => m && m.hp > 0)
            ...
            if (scene.doorRects && !scene._waveCharging.active) {
              for (const dr of scene.doorRects) {
                ...
                if (
                  scene.player.x >= b.left &&
                  scene.player.x <= b.right &&
                  scene.player.y >= b.top &&
                  scene.player.y <= b.bottom
                ) {
                  insideAnyDoor = true
                  ...
                  if (hasAliveMonsters) {
                    ...
                    const pushBack = 10
                    const opp = {
                      north: { x: 0, y: pushBack },
                      south: { x: 0, y: -pushBack },
                      west: { x: pushBack, y: 0 },
                      east: { x: -pushBack, y: 0 },
                    }
            ...
            if (
              scene.encounterData &&
              (!scene.encounterData.used || scene.encounterData.type === 'FOUNTAIN')
            ) {
            ...
            try {
              if (
                scene.keys.ONE &&
                Phaser.Input.Keyboard.JustDown(scene.keys.ONE) &&
                !scene._waveCharging.active
              ) {
                ;(async () => {
                  try {
            ...
            try {
              if (
                scene.keys.TWO &&
                Phaser.Input.Keyboard.JustDown(scene.keys.TWO) &&
                !scene._waveCharging.active
              ) {
                ;(async () => {
                  try {
            ...
            try {
              if (
                scene.keys.THREE &&
                Phaser.Input.Keyboard.JustDown(scene.keys.THREE) &&
                !scene._waveCharging.active
              ) {
                ;(async () => {
                  try {
            ...
            try {
              if (
                scene.keys.FOUR &&
                Phaser.Input.Keyboard.JustDown(scene.keys.FOUR) &&
                !scene._waveCharging.active
              ) {
                ;(async () => {
                  try {
            ...
            try {
              if (
                scene.keys.FIVE &&
                Phaser.Input.Keyboard.JustDown(scene.keys.FIVE) &&
                !scene._waveCharging.active
              ) {
                ;(async () => {
                  try {
                    ...
                    ;(async () => {
                      try {
                        ...
                        if (
                          ...
                          // 宝箱事件：显示浮动金币文本
                          if (rewardGold > 0) {
                            const flyText = scene.add
                              .text(eventX, eventY - 40, '你获得了' + rewardGold + '枚金币！', {
