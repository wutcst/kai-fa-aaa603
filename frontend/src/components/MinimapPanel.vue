...
<script setup>
  ...
  function drawMinimap(highlightRoomName) {
    ...
    for (const room of rooms) {
      ...
      for (const [dir, neighborName] of Object.entries(exits)) {
        ...
        const edgeId =
          room.name < neighborName
            ? `${room.name}->${neighborName}`
            : `${neighborName}->${room.name}`
        ...
        switch (dir) {
          case 'north':
            x1 = rectA.x + rectA.w / 2
            y1 = rectA.y
            x2 = rectB.x + rectB.w / 2
            y2 = rectB.y + rectB.h
            break
          case 'south':
            x1 = rectA.x + rectA.w / 2
          ...
          case 'west':
            x1 = rectA.x
            y1 = rectA.y + rectA.h / 2
            x2 = rectB.x + rectB.w
            y2 = rectB.y + rectB.h / 2
            break
          case 'east':
            x1 = rectA.x + rectA.w
            y1 = rectA.y + rectA.h / 2
  ...
  async function initMinimap() {
    ...
    try {
      ...
      console.log(
        '[Minimap] API response rooms:',
        data.rooms ? data.rooms.map((r) => ({ name: r.name, roomType: r.roomType })) : 'none',
      )
